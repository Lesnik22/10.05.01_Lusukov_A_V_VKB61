...
class User(UserMixin, db.Model):
    ...
    # department = db.Column(db.String(100), nullable=True)  # Временно отключено
...
@login_required
def dashboard():
    # Файлы текущего пользователя (всегда видны владельцу)
    user_files = File.query.filter_by(owner_id=current_user.id).order_by(File.created_at.desc()).all()

    # Файлы, к которым предоставлен явный доступ (FileAccess)
    explicitly_shared_files = File.query.join(FileAccess).filter(
    ...
    shared_map = {}
    for f in explicitly_shared_files + public_files + department_files:
        shared_map[f.id] = f
...
def download_file(file_id):
    try:
        ...
        # Проверяем доступ на уровне отдела
              ...
              file.department == current_user.department and
              current_user.role.has_permission('download_files')):
            has_access = True
...
def share_file(file_id):
    ...
    # При приватном доступе даём точечный доступ, при публичном доступе шаринг не нужен
...
def admin_panel():
    ...
    return render_template('admin.html', users=users, files=files, audit_logs=audit_logs)
